...
interface MobileAuthResumeUploadProps {
...
export default function MobileAuthResumeUpload({ onUploadSuccess, onUploadError, onFileSelected }: MobileAuthResumeUploadProps) {
  ...
  return (
    <div className="w-full">
      {!selectedFile || uploadStatus === 'error' ? (
        /* Upload Form - Simplified for auth modal */
        <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-6 text-center">
          <div className="w-12 h-12 mx-auto mb-4 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center">
            <Upload className="w-6 h-6 text-gray-400" />
          </div>
          <p className="text-gray-600 dark:text-gray-400 mb-2 text-lg">Upload Resume</p>
          <p className="text-sm text-gray-500 dark:text-gray-500 mb-4">PDF files only</p>
          ...
          <input
            ...
            className="hidden"
            id="mobile-auth-resume-input"
          />
          
          <button
            onClick={() => fileInputRef.current?.click()}
            className="px-6 py-3 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg cursor-pointer transition-colors"
          >
            Choose File
          </button>
